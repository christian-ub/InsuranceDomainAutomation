*** Settings ***
Documentation   SQL scripts for TestCase TC_03_05: Total Sum_Insured_Total Queensland clients over $250000
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_03_05
    set test variable    ${Test_Column_1}    Result
    set test variable    ${TestStatus}    Pending

Total Sum_Insured_Total Queensland clients over $250000    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  Query    select sum(Sum_Insured_Total) from InsuranceDataDomain.fact_insurance_cover f join InsuranceDataDomain.dim_client d on f.Client_SKey=d.Client_SKey where d.State = 'Queensland' and f.Sum_Insured_Total > 250000
    Set test Variable  ${TestValue_Column_1}  ${QueryResults[0][0]}
...
Save Current Row Count    # Automated
    ${current_row_count} =  Row Count   SELECT * FROM TestScenario_03 WHERE TestCase = '${TestCase}'
...
Insert New Data    # Automated for one value of Test_Column and TestValue_Column, manual insert for multiple columns until list in place
    ...
    Execute SQL String  INSERT INTO TestScenario_03 (TestCase, DateStamp, Test_Status, ${Test_Column_1}) VALUES ('${TestCase}', '${Insert_DateStamp}', 'PENDING', ${TestValue_Column_1})

Verify New Data Added    # Automated
    ${new_row_count} =  Row Count  SELECT * FROM TestScenario_03 WHERE TestCase = '${TestCase}'
    Log  ${NEW_ROW_COUNT}
...
Verify Last Input    # Manual insert of Test_Columns and TestValue_Columns and verification rules for correctness, Automated once list in place
    ${QueryResults} =  Query  SELECT TestCase, DateStamp, ${Test_Column_1} FROM TestScenario_03 WHERE TestCase = '${TestCase}' ORDER BY DateStamp DESC LIMIT 1
...
Update Newly Inserted Data    # Automated for one value, manual insert for multiple columns until list in place
    ...
    ...    Execute SQL String  UPDATE TestScenario_03 SET Test_Status = 'PASSED' WHERE TestCase = '${TestCase}' AND DateStamp = '${Insert_DateStamp}'
    run keyword if    ${TestStatus}!= 'Verified Insert Data'
    ...    Execute SQL String  UPDATE TestScenario_03 SET ${Test_Column_1} = NULL, Test_Status = 'FAILED' WHERE TestCase = '${TestCase}' AND DateStamp = '${Insert_DateStamp}'
    fail    Corrupt Insert Data

Log All Rows    # Automated
    Log    ${TestStatus}
    ${QueryResults} =  Query  SELECT * FROM TestScenario_03 WHERE TestCase = '${TestCase}' ORDER BY DateStamp DESC
