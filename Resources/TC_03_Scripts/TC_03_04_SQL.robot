*** Settings ***
Documentation   SQL scripts for TestCase TC_03_04: Total Sum_Insured_Total Queensland clients
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_03_04
    set test variable    ${Test_Column_1}    Result
    set test variable    ${TestStatus}    Pending

Total Sum_Insured_Total Queensland clients    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  Query    select sum(Sum_Insured_Total) from InsuranceDataDomain.fact_insurance_cover f join InsuranceDataDomain.dim_client d on f.Client_SKey=d.Client_SKey where d.State = 'Queensland'
