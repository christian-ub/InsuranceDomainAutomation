*** Settings ***
Documentation   SQL scripts for TestCase TC_03_07: Count of Queensland clients with over 4 Units_Total
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_03_07
    set test variable    ${Test_Column_1}    Result
    set test variable    ${TestStatus}    Pending

Count of Queensland clients with over 4 Units_Total    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    select * from InsuranceDataDomain.fact_insurance_cover f join InsuranceDataDomain.dim_client d on f.Client_SKey=d.Client_SKey where d.State = 'Queensland' and f.Units_Total >=4
