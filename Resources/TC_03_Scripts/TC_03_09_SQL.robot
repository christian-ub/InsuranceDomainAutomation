*** Settings ***
Documentation   SQL scripts for TestCase TC_03_09: Check if any units are negative
...
Set Variables    # Manual insert of TestCase and Test_Columns
    set test variable    ${TestCase}    TC_03_09
    set test variable    ${Test_Column_1}    Result
    set test variable    ${TestStatus}    Pending

Check if any units are negative    # Manual insert of SQL query and handling of variables
    ${QueryResults} =  row count    select * from InsuranceDataDomain.fact_insurance_cover where Default_Units < 0 or Additional_Units < 0 or Units_Total < 0
...
Verify Retrieved Data    # Manual insert of verification rules for correctness
    should not be true    ${TestValue_Column_1}
